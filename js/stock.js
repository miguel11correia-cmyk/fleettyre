...
function selecionarDeStock(linhaId, marca, medida, tipo, fornecedor, preco) {
  const campos = [
    ['r-marca', marca], ['r-medida', medida], ['r-tipo', tipo],
    ['r-forn', fornecedor], ['r-custo', preco > 0 ? preco : '']
  ];
  ...
  if (info) { info.textContent = '✓ Pneu de fatura: ' + marca + ' ' + medida + ' ' + tipo + (preco > 0 ? ' — ' + fmtEur(preco) : '') + ' — ' + fornecedor; info.classList.remove('hidden'); }
  fecharSelStock();
}

function selecionarDesmontadoDeStock(id, marca, medida, tipo, matriculaOrigem, tabela) {
  const campos = [
    ['r-marca', marca], ['r-medida', medida], ['r-tipo', tipo], ['r-forn', 'PARQUE'], ['r-custo', '']
  ];
  campos.forEach(([fid, val]) => { const el = document.getElementById(fid); if (el) el.value = val; });
