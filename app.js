...
async function guardarRegisto() {
  ...
  // Marcar pneu desmontado como remontado se foi seleccionado de armazém
  if (stockDesmontadoSelId) {
    await sb.from('pneus').update({ remontado: true }).eq('id', stockDesmontadoSelId);
...
// ── STOCK — PNEUS DESMONTADOS ─────────────────────────────────────

/**
 * Carrega pneus desmontados com destino "Abrir Piso" ou "Stock"
 * que ainda não foram remontados (sem registo de nova montagem após desmontagem)
 * Lógica: buscar todos os registos com mes_desmont preenchido
 * e destino "Abrir Piso" ou "Stock"
 * e que não têm um registo de remontagem posterior
 */
...
/**
 * Renderiza secção de pneus desmontados em armazém na página de Stock
 */
...
/**
 * Seleccionar pneu desmontado de armazém para nova montagem
 * Preenche marca, medida e tipo — sem custo (vai ser definido depois)
 * e sem fornecedor (é um pneu da própria empresa)
 */
